...
tlength = 15000;  % This should be 15000
...
%% Ridge from SVD directly

[u,w,v] = svd(rawStim_vector);
w2 = diag(w).*diag(w);
w2inv = 1./w2;

filter_from_svd = v*diag(w2inv)*v'*cross_stim_response;

figure(17);
subplot(2,1,1);
imagesc(reshape(filter_from_svd, [10 10]));
colormap('gray');
title('Filter obtained by svd');
axis image;
subplot(2,1,2);
imagesc(gabor);
colormap('gray');
...
for il=1:length(l);
    dinv_l{il} = diag(1./(w2 + l(il)));
    myfiltersvd_l{il} = v*dinv_l{il}*v'*cross_stim_response;
end
